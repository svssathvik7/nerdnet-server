...
const addComment = async (req, res) => {
  try {
    ...
    if (postMatch && userMatch) {
      ...
      await session
        .withTransaction(async () => {
          ...
          res
            .status(200)
            .json({
              commentUpdate: "Successfully added comment!",
              status: true,
            });
        })
        .catch((error) => {
      ...
      res
        .status(404)
        .json({ commentUpdate: "Post or user not found!", status: false });
    }
  } catch (error) {
    console.log(error);
    res
      .status(500)
      .json({ commentUpdate: "Something went wrong!", status: false });
  }
};
...
const changeLikes = async (req, res) => {
  try {
    ...
    if (postMatch && userMatch) {
      if (addLike) {
        const alreadyExists = await postMatch.likes.some(
          (like) => like?._id === userLiked?._id
        );
        if (!alreadyExists) {
        ...
        const alreadyExists = await postMatch.dislikes.some(
          (dislike) => dislike?._id === userLiked?._id
        );
        if (!alreadyExists) {
          var postUpdated = await postDb.updateOne(
            { _id: postId },
    ...
    await session.withTransaction(async () => {
      ...
      if (postMatch && userMatch) {
        ...
        res
          .status(404)
          .json({
            message: "Data inconsistency! Critical Db error!!",
            status: false,
          });
      }
    });
